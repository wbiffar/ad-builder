...
export function MobileLeaderboard({ config, adRef }: AdTemplateProps) {
  const { colors, logoUrl, tagline, ctaText, designElements, logoSettings } = config;
  ...
  return (
    <div ref={adRef} style={{ width: WIDTH, height: HEIGHT, background: bg, position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 10px", gap: 8, fontFamily: "'Inter', 'DM Sans', sans-serif", ...borderStyles }}>
      ...
      <div style={{ position: "relative", zIndex: 3, display: "flex", alignItems: "center", gap: 8, width: "100%", justifyContent: "space-between" }}>
        <div style={{ order: order.logoOrder, flexShrink: 0 }}>{logoUrl && <LogoImage src={logoUrl} maxWidth={60} maxHeight={35} whiteContainer={wc} />}</div>
        <div style={{ color: textColor, fontSize: 11, fontWeight: 600, lineHeight: 1.2, flex: 1, textAlign: "center", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", order: 1 }}>{tagline}</div>
        <div style={{ order: order.ctaOrder, flexShrink: 0 }}><CtaButton text={ctaText} bgColor={colors.accent} fontSize={9} padding="4px 10px" /></div>
      </div>
    </div>
  );
}
